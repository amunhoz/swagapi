...
var find = async function (ctx, returnResult) {
    ...
    */
    ...
    if (ctx.req.query) query = swagapi.lib.blueprints.helper.util.mergeQuery (query, ctx.req.query)
    ...
    if (ctx.subItens && result[0]) {
        ...
        for(var i = 0; i < result.length;i++){
            ...
            if (ctx.subItens.addFilter) squery.where  = swagapi.lib.blueprints.helper.util.AddAndFilter(squery.where, ctx.subItens.addFilter);
